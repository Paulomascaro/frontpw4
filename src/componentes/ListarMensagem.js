...
export function ListarMensagem() {
    ...
    return (
        <div className='lista mx-8'>
            <h3 className="text-3xl font-bold mt-2 sm:mx-auto">Listagem de Mensagens</h3>
            <table className="min-w-full">
                ...
                {mensagens.map((mensagem) => {
                    return (
                        <tr onClick={() => {
                            window.location.href = `/mensagem/${mensagem.id}`
                        }} key={mensagem.id}>
                            <td className="text-center">{mensagem.id}</td>
                            <td className="text-center">{mensagem.msg}</td>
                            <td className="grid grid-cols-3 w-32 justify-end">
                                <svg onClick={(event) => {
                                ...
                                <svg onClick={(event    ) => {
                                    event.stopPropagation();
                                    window.location.href = `/novaresposta/${mensagem.id}`
                                }} xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-6 h-6">
